...
DEFINE_EXECUTE_AT_END(DomainCreater)/*Domain creater produces a matrix just like the geometry and puts in every cell the cell cell_t, thread and the cell centroid*/
{
  ...
  d=Get_Domain(2);
FILE *fp; fp=fopen("Domain Creator.txt","a");
  int i=332;int j=0; 
  thread_loop_c(t,d)
    {
      begin_c_loop(c,t)
	{
	 ...
	 fprintf(fp,"current cell : ( %g,  %g)       vof: %g\n", xc[0], xc[1], C_VOF(c,t));
	  i=i-1;
	  if(i==-1)
	    {
	      i=332;
...
{ 
  ...
  thread_loop_c(t,d)
      ...
      begin_c_loop(c,t)
	{
	  ...
	  C_CENTROID(xc,c,t);
	  node1 = C_NODE(c,t,0);	  node2 = C_NODE(c,t,1);	  node3 = C_NODE(c,t,2);	  node4 = C_NODE(c,t,3);
	       if (xc[0]>NODE_X(node1) && xc[1]<NODE_Y(node1)){x1[0]=NODE_X(node1);x1[1]=NODE_Y(node1);}
	  else if (xc[0]>NODE_X(node2) && xc[1]<NODE_Y(node2)){x1[0]=NODE_X(node2);x1[1]=NODE_Y(node2);}
	  else if (xc[0]>NODE_X(node3) && xc[1]<NODE_Y(node3)){x1[0]=NODE_X(node3);x1[1]=NODE_Y(node3);}
	  else if (xc[0]>NODE_X(node4) && xc[1]<NODE_Y(node4)){x1[0]=NODE_X(node4);x1[1]=NODE_Y(node4);}

	       if (xc[0]<NODE_X(node1) && xc[1]<NODE_Y(node1)){x2[0]=NODE_X(node1);x2[1]=NODE_Y(node1);}
	  else if (xc[0]<NODE_X(node2) && xc[1]<NODE_Y(node2)){x2[0]=NODE_X(node2);x2[1]=NODE_Y(node2);}
	  else if (xc[0]<NODE_X(node3) && xc[1]<NODE_Y(node3)){x2[0]=NODE_X(node3);x2[1]=NODE_Y(node3);}
	  else if (xc[0]<NODE_X(node4) && xc[1]<NODE_Y(node4)){x2[0]=NODE_X(node4);x2[1]=NODE_Y(node4);}

	       if (xc[0]<NODE_X(node1) && xc[1]>NODE_Y(node1)){x3[0]=NODE_X(node1);x3[1]=NODE_Y(node1);}
	  else if (xc[0]<NODE_X(node2) && xc[1]>NODE_Y(node2)){x3[0]=NODE_X(node2);x3[1]=NODE_Y(node2);}
	  else if (xc[0]<NODE_X(node3) && xc[1]>NODE_Y(node3)){x3[0]=NODE_X(node3);x3[1]=NODE_Y(node3);}
	  else if (xc[0]<NODE_X(node4) && xc[1]>NODE_Y(node4)){x3[0]=NODE_X(node4);x3[1]=NODE_Y(node4);}

	       if (xc[0]>NODE_X(node1) && xc[1]>NODE_Y(node1)){x4[0]=NODE_X(node1);x4[1]=NODE_Y(node1);}
	  else if (xc[0]>NODE_X(node2) && xc[1]>NODE_Y(node2)){x4[0]=NODE_X(node2);x4[1]=NODE_Y(node2);}
	  else if (xc[0]>NODE_X(node3) && xc[1]>NODE_Y(node3)){x4[0]=NODE_X(node3);x4[1]=NODE_Y(node3);}
	  else if (xc[0]>NODE_X(node4) && xc[1]>NODE_Y(node4)){x4[0]=NODE_X(node4);x4[1]=NODE_Y(node4);}
